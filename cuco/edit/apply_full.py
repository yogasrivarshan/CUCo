...
from cuco.llm import extract_between
import logging
...
def apply_full_patch(
    ...
    try:
        ...
        if patch_has_both:
            ...
            if len(patch_mutable_ranges) != len(mutable_ranges):
                logger.warning(
                    f"apply_full_patch: block count mismatch — "
                    f"original has {len(mutable_ranges)}, "
                    f"patch has {len(patch_mutable_ranges)}"
                )
                error_message = (
                    f"EVOLVE-BLOCK count mismatch: original has "
                    f"{len(mutable_ranges)} block(s), patch has "
                    f"{len(patch_mutable_ranges)} block(s)"
                )
                return original, 0, None, error_message, None, None
