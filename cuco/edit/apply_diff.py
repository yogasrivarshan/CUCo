...
def _mutable_ranges(text: str) -> list[tuple[int, int]]:
    ...
    Handles both sequential and nested EVOLVE-BLOCK markers correctly
    by interleaving START/END events in file-position order and using
    a stack for proper bracket matching.
    """
    events: list[tuple[int, str, int]] = []
    for m in EVOLVE_START.finditer(text):
        events.append((m.start(), "start", m.end()))
    for m in EVOLVE_END.finditer(text):
        events.append((m.start(), "end", m.start()))
    events.sort(key=lambda e: e[0])

    spans: list[tuple[int, int]] = []
    stack: list[int] = []
    for _pos, kind, boundary in events:
        if kind == "start":
