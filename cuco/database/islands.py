...
class MultiSeedIslandStrategy(IslandStrategy):
    """Island strategy for multi-seed evolution where each island gets a
    different initial program. Does NOT auto-copy the first program to all
    islands; generation 0 seeds each island explicitly."""
    ...
    def assign_island(self, program: Any) -> None:
        ...
        # If island_idx is already set (explicit seeding from _run_generation_0),
        # respect it.
        if program.island_idx is not None:
            return

        # Children inherit parent's island.
        if program.parent_id:
            self.cursor.execute(
                "SELECT island_idx FROM programs WHERE id = ?", (program.parent_id,)
            )
        ...
        # Fill empty islands first.
        initialized = set(self.get_initialized_islands())
        empty = [i for i in range(num_islands) if i not in initialized]
        if empty:
            program.island_idx = min(empty)
            return

        # Fallback: island with fewest programs.
        self.cursor.execute(
            "SELECT island_idx, COUNT(*) as cnt FROM programs "
            "WHERE island_idx IS NOT NULL GROUP BY island_idx"
        )
        pops = {row["island_idx"]: row["cnt"] for row in self.cursor.fetchall()}
        program.island_idx = min(range(num_islands), key=lambda i: pops.get(i, 0))
...
class DirectionalMigrationStrategy(ElitistMigrationStrategy):
    """Migration strategy that only migrates along directed edges.
    Uses a migration_graph: Dict[int, List[int]] where keys are source islands
    and values are lists of allowed destination islands.
    E.g. {0: [2], 1: [2], 2: []} means islands 0 and 1 can only send to island 2,
    and island 2 never sends migrants out."""
    ...
    def __init__(
        ...
        conn: sqlite3.Connection,
        config: Any,
        migration_graph: Optional[Dict[int, List[int]]] = None,
    ):
        super().__init__(cursor, conn, config)
        self.migration_graph = migration_graph or getattr(
            config, "migration_graph", None
        ) or {}
    ...
    def perform_migration(self, current_generation: int) -> bool:
        ...
        logger.info(
            f"Performing directional migration at generation {current_generation}"
        )
        ...
        for source_idx in range(num_islands):
            dest_islands = self.migration_graph.get(source_idx, [])
            if not dest_islands:
                continue
            ...
            for migrant_id in migrants:
                if migrant_id in all_migrated_programs:
                    continue
                all_migrated_programs.add(migrant_id)
                dest_idx = random.choice(dest_islands)
                self._migrate_program(
        ...
        logger.info(
            f"Directional migration complete. Migrated {total_migrated} programs."
        )
        return total_migrated > 0
...
class CombinedIslandManager:
    ...
    def assign_island(self, program: Any) -> None:
        ...
        self._apply_api_type_tag(program)

    def _apply_api_type_tag(self, program: Any) -> None:
        """Set metadata['api_type'] based on island_api_types config mapping."""
        island_api_types = getattr(self.config, "island_api_types", None)
        if not island_api_types or program.island_idx is None:
            return
        api_type = island_api_types.get(program.island_idx)
        if api_type:
            if program.metadata is None:
                program.metadata = {}
            program.metadata["api_type"] = api_type
