...
def get_client_model(model_name: str) -> tuple[Union[openai.OpenAI, str], str]:
    if model_name in OPENAI_EMBEDDING_MODELS:
        ...
        # Bedrock's OpenAI-compatible endpoint does NOT support /embeddings.
        # Use the native boto3 InvokeModel API instead.
    ...
    def get_embedding(
        ...
        if self.model_name in GEMINI_EMBEDDING_MODELS:
            try:
                ...
                if single_code:
                    ...
                    return [[]], 0.0
        # Handle Bedrock models via native boto3 InvokeModel API
        if self.model_name.startswith("bedrock"):
            try:
                ...
                cost = 0.0  # Titan Embed pricing is negligible
                if single_code:
                    return embeddings[0] if embeddings else [], cost
                else:
                    return embeddings, cost
        ...
        try:
            ...
            total_tokens = getattr(response.usage, "total_tokens", None) if response.usage else 0
            total_tokens = total_tokens or 0
            cost_per_token = OPENAI_EMBEDDING_COSTS.get(self.model, 0.0)
            cost = total_tokens * cost_per_token
            if response.data is None or len(response.data) == 0:
                logger.warning(
                    f"Embedding response had no data (model={self.model}). "
